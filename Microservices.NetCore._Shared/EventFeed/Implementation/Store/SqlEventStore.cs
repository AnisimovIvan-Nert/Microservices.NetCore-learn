    ...
    public class SqlEventStore : IEventStore
    {
        ...
        public async ValueTask<IEnumerable<Event>> GetEvents(long firstNumber, long lastNumber)
        {
            ...
            const string readEventsSql =
                $"""
                 SELECT
                     OW_NUMBER() OVER (PARTITION BY nameof(@params.Type)ORDER BY ID) RowNumber,
                     * 
                 FROM EventStore 
            ...
            {
                var sequenceNumber = value.RowNumber;
                var name = value.Name;
                var type = value.Type;
                var occurredAt = value.OccurredAt;

                var contentJson = value.Content;
                var content = JsonConvert.DeserializeObject(contentJson);

                return new Event(sequenceNumber, occurredAt, name, content, type);
            }
        }
